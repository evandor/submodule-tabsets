  ...
  <q-dialog ref="dialogRef" @hide="onDialogHide">
    <q-card class="q-dialog-plugin">
      ...
      <q-card-section class="q-pt-none" v-if="importFrom === 'github'">
        <span>using repository<br>
          <em>{{ LocalStorage.getItem(GITHUB_USERNAME) }}/{{ LocalStorage.getItem(GITHUB_REPONAME) }}/{{ LocalStorage.getItem(GITHUB_PATH) }}</em></span>
      </q-card-section>
...
const importData = async () => {
  if (importFrom.value === "json") {
    ...
    useCommandExecutor().executeFromUi(new ImportTabsetsCommand(fromGithub.result))
      .then(() => {
        sendMsg('tabsets-imported', {})
      })
  }
